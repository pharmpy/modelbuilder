...
elim_button = dbc.Button("Elimination rate", id="elim-btn", color=btn_color, n_clicks=0, size="md")
...
model_format_button = dbc.Button(
    "Format", id="format-button", color=btn_color, n_clicks=0, size="md"
)

model_type_button = dbc.Button("Type", id="type-button", color=btn_color, n_clicks=0, size="md")

route_button = dbc.Button("Route", id="route-button", color=btn_color, n_clicks=0, size="md")


# NOTE: not that good if used in collapse
button_group = dbc.ButtonGroup([model_format_button, model_type_button, route_button])
...
# COLLAPSABLES
collapse_err = dbc.Collapse(error_dropdown, id="collapse_err", is_open=True)

collapse_elimination = dbc.Collapse(elim_dropdown, id="collapse_elim", is_open=True)

collpasable_button_group = dbc.Collapse(button_group, id="collapse_bg", is_open=True)
                ...
                children=[
                    ...
                    dbc.InputGroup(
                        [
                            ...
                            dbc.Input(
                                id="pk_iiv_inp",
                                placeholder="Change init_est (=0.09)?",
                                type="number",
                                style={"width": "50%"},
                            ),
                        ]
...
abs_rates = dbc.Col(children=[abs_button, abs_rate_collapse])

elim_rates = dbc.Col(children=[elim_button, collapse_elimination])

error_rates = dbc.Col(children=[error_button, collapse_err])
