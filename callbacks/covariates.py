...
def covariate_callbacks(app):
    ...
    def get_params_covars(tab):
            ...
            def param_expression (parameter):
                return [parameter+": ", str(config.model.statements.before_odes.full_expression(parameter))]

            if tab == "covariate-tab":
                try:
                    covar_names = get_model_covariates(config.model, strings=True) if get_model_covariates(config.model, strings=True) else config.model.datainfo.typeix["unknown"].names
                    covar_opt = [make_label_value(covar,covar) for covar in covar_names]
                    param_opt = [make_label_value(param,param) for param in get_individual_parameters(config.model)]
                    render = dbc.ListGroup(children=[dbc.ListGroupItem(param_expression(item)) for item in get_individual_parameters(config.model)]) 
            ...
            if button:
                if custom:
                    config.model= add_covariate_effect(
                        config.model, parameter, covariate, custom, operation=operation, allow_nested= nestle=="True")
                config.model= add_covariate_effect(
                    config.model, parameter, covariate, effect, operation=operation, allow_nested= nestle=="True")
                return dbc.ListGroup(children=[dbc.ListGroupItem(param_expression(item)) for item in get_individual_parameters(config.model)]), 0 
            return dbc.ListGroup(children=[dbc.ListGroupItem(param_expression(item)) for item in get_individual_parameters(config.model, )]) , 0
        except: raise PreventUpdate
    return